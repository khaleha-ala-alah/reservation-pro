    ...
    <div className="space-y-6 animate-fade">
      ...
      <div className="card card-pad">
        ...
        <p className="text-sm text-slate-500">
          Capacité: {equipment.capacity} • Localisation: {equipment.location} • Statut: {equipment.status}
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <div className="card card-pad">
          ...
          <EquipmentCalendar
            ...
            onSelect={(start, end) => {
              ...
              if (hasConflict(equipment.id, s, e)) {
                setToast({ text: "Conflit détecté : créneau déjà occupé.", kind: "error" });
                return;
              }
        ...
        <div className="card card-pad">
          ...
          {reservations.length === 0 ? (
            <div className="text-slate-600">Aucune réservation pour cet équipement.</div>
          ) : (
              ...
              {reservations.map((r) => (
                <li key={r.id} className="flex items-center justify-between border rounded-lg p-3">
                  <div>
                    ...
                    <div className="text-slate-500">Utilisateur: {r.userId} • Statut: {r.status}</div>
                    {r.reason && <div className="text-slate-600">Motif: {r.reason}</div>}
                  </div>
                  {r.status !== "cancelled" && (
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="card card-pad w-full max-w-md shadow-xl animate-fade">
              <h3 className="text-xl font-semibold text-center">Motif de la réservation</h3>
              <p className="text-gray-500 text-sm text-center mt-1 mb-4">(facultatif)</p>
